...
namespace DotNetARX
{
    ...
    public static class LayoutTools
    {
        ...
        /// <param name="includeFirstViewport">是否将第一个视口包含在内</param>
        /// <returns>返回所有实体的Id集合</returns>
        public static ObjectIdCollection GetEntsInLayout(this Layout layout, bool includeFirstViewport)
        {
            ObjectIdCollection entIds = new ObjectIdCollection();
            ObjectId id = layout.BlockTableRecordId;
            ...
            foreach (ObjectId entId in btr)
            {
                ...
                if (vp != null && firstViewport)
                {
                    if (includeFirstViewport)
                        entIds.Add(entId);
                    firstViewport = false;
                }
                else
                    ...
                    if (btr.IsLayout)
                    {
                        ...
                        if (layout.LayoutName.CompareTo(layoutName) == 0)
                        {
                            ...
                            if (firstViewportId.IsValid && secondViewportId.IsValid)
                            {
                                ...
                                Point3d midPt = GeoTools.MidPoint(ext.MinPoint, ext.MaxPoint);
                                firstVP.ViewCenter = new Point2d(midPt.X, midPt.Y);
                                double scaleX = secondVP.Width / ((ext.MaxPoint.X - ext.MinPoint.X) * 1.1);
                                double scaleY = secondVP.Height / ((ext.MaxPoint.Y - ext.MinPoint.Y) * 1.1);
                                firstVP.CustomScale = Math.Min(scaleX, scaleY);
                            }
                        }
                    }
                }
