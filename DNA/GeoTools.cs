...
namespace DotNetARX
{
    ...
    /// 几何类
    /// </summary>
    public static partial class GeoTools
    {
        /// <summary>
        /// 将UCS中的块的范围转换为实际的位置
        /// </summary>
        /// <param name="ed"></param>
        /// <param name="ent">块参照</param>
        /// <param name="isPaperSpace">是否在图纸空间</param>
        /// <returns>返回实际的块的位置</returns>
        public static Extents2d TranslateUCSExtents(this Editor ed, Entity ent, bool isPaperSpace)
        {
            //打印窗口
            Point3d ptMin = ent.GeometricExtents.MinPoint;
            Point3d ptMax = ent.GeometricExtents.MaxPoint;
            //转换到UCS
            ptMin = ptMin.TransformBy(ed.CurrentUserCoordinateSystem.Inverse());
            ptMax = ptMax.TransformBy(ed.CurrentUserCoordinateSystem.Inverse());
            //转换到DCS
            ptMin = Autodesk.AutoCAD.Internal.Utils.UcsToDisplay(ptMin, isPaperSpace);
            ptMax = Autodesk.AutoCAD.Internal.Utils.UcsToDisplay(ptMax, isPaperSpace);
            //得到打印范围
            return new Extents2d(ptMin.Convert2d(new Plane()), ptMax.Convert2d(new Plane()));
        }
